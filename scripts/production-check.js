...
const e=fs.readFileSync(env,'utf8');for(const key of ['ZOVRO_SECRET','ZOVRO_OPS_TOKEN','ZOVRO_ALLOWED_ORIGINS','STRIPE_PUBLISHABLE_KEY','STRIPE_SECRET_KEY','STRIPE_WEBHOOK_SECRET','ONESIGNAL_APP_ID','ONESIGNAL_REST_API_KEY','DATABASE_URL','ZOVRO_DB_MIRROR_MODE','ZOVRO_PLATFORM_FEE_BPS','ZOVRO_SMS_ENABLED','ZOVRO_SMS_INBOUND_URL'])if(!e.includes(key)){console.error('Missing production variable documentation:',key);bad=true}
const core=fs.readFileSync(path.join(root,'backend','server-core.js'),'utf8');
const d=fs.readFileSync(path.join(root,'backend','database.js'),'utf8');if(!core.includes('SECRET.length<32')||!core.includes('OPS_TOKEN.length<24')){console.error('Production secret guards missing');bad=true}
