...
for(const status of ['On the way','Arrived'])await call('POST',`/api/requests/${id}/status`,{status},p2.token);
const customerViewBeforeWork=(await call('GET','/api/requests',null,c.token)).requests.find(x=>x.id===id);assert(customerViewBeforeWork?.securityCode,'Customer must receive the service verification code');
const missingServiceCode=await fetch(base+`/api/requests/${id}/status`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${p2.token}`},body:JSON.stringify({status:'In progress'})});assert.equal(missingServiceCode.status,409,'Work must not begin without the customer verification code');
await call('POST',`/api/requests/${id}/status`,{status:'In progress',securityCode:customerViewBeforeWork.securityCode},p2.token);
await call('POST',`/api/requests/${id}/status`,{status:'Completed'},p2.token);
