...
(async()=>{
 ...
 f=setup([ok=>ok(position())],false);f.c.window.Capacitor={isNativePlatform:()=>true};await f.run();assert.equal(f.calls,1,'Native Capacitor origin must retain OS geolocation support');
