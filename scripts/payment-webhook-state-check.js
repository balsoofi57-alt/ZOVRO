...
(async()=>{
  ...
  state.requests.push({id:'req_failed',stripePaymentIntentId:'pi_failed',paymentState:'payment_pending'});
  const failed=event('evt_failed_now','payment_intent.payment_failed',{id:'pi_failed',last_payment_error:{code:'card_declined',message:'Card declined'},metadata:{zovro_request_id:'req_failed'}});
  const failedReply=await send(failed);
  assert.equal(failedReply.status,200,'Signed payment_failed webhook must be acknowledged');
  const failedRequest=state.requests.find(r=>r.id==='req_failed');
  assert.equal(failedRequest.paymentState,'failed');
  assert.equal(failedRequest.paymentFailureCode,'card_declined');
  assert.equal(failedRequest.paymentFailureMessage,'Card declined');
  const failedOnce=JSON.stringify(failedRequest);
  const failedReplay=await send(failed);
  assert.equal(failedReplay.status,200);
  assert.equal(failedReplay.body.duplicate,true,'Repeated payment_failed event must be deduplicated');
  assert.equal(JSON.stringify(state.requests.find(r=>r.id==='req_failed')),failedOnce,'Repeated payment_failed event must not mutate state');
  ...
  assert.equal(state.requests.find(r=>r.id==='req_other').paymentState,'paid_held','Conflicting identifiers must not update a request');
  state.requests.find(r=>r.id==='req_other').paymentState='released';
  await send(event('evt_late_failed','payment_intent.payment_failed',{id:'pi_other',metadata:{zovro_request_id:'req_other'}}));
  assert.equal(state.requests.find(r=>r.id==='req_other').paymentState,'released');
  state.audit=[];
