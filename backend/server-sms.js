...
function twiml(res,text){const escaped=String(text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');res.writeHead(200,{'content-type':'text/xml; charset=utf-8','cache-control':'no-store'});res.end(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escaped}</Message></Response>`);}
...
function audit(db,userId,action,meta={}){db.audit.unshift({id:crypto.randomUUID(),user:userId||null,action,meta,at:new Date().toISOString()});db.audit=db.audit.slice(0,1000);}
...
async function deliverPrepared(jobs){
 ...
 for(const job of jobs){
  try{const result=await sms.sendJobOffer({smsProvider:gateway,...job});console.log(JSON.stringify({event:'twilio.job_offer',offerId:job.offer.id,providerId:job.user.id,skipped:!!result.skipped,status:result.status||null,reason:result.reason||null}));}
...
async function inbound(req,res,url){
 ...
 const messageSid=String(params.MessageSid||params.SmsSid||''),key=sms.replyIdempotencyKey(messageSid),from=sms.normalizeE164(params.From),reply=sms.classifyReply(params.Body);
 if(!messageSid||!from)return json(res,400,{error:'Missing Twilio message identity'});
 const db=readDb();
 if(db.audit.some(row=>row.action==='twilio.sms.inbound'&&row.meta?.idempotencyKey===key))return twiml(res,'Your reply was already received.');
 ...
 const row=latestOffer(db,provider.id);
