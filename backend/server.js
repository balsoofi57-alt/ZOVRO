...
(async()=>{
  ...
  if(/^(1|true|yes|on)$/i.test(String(process.env.ZOVRO_PAYOUT_READINESS_PROBE||''))){
    const {runPayoutReadinessProbe}=require('./payout-readiness-probe');
    await runPayoutReadinessProbe();
  }
