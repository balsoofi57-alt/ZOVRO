...
const keyMap={users:'users',service_requests:'requests',messages:'messages',ratings:'ratings',audit_log:'audit',verification_requests:'verificationRequests',provider_locations:'providerLocations',device_sessions:'deviceSessions',notifications:'notifications'};
...
async function load(){if(!enabled())return null;return withClient(async c=>{const state={schemaVersion:5};let total=0;for(const [table] of tables){const r=await c.query(`SELECT data FROM ${table}`);const rows=r.rows.map(x=>x.data);state[keyMap[table]]=rows;total+=rows.length}return {state,total}})}
function placeholders(n){return Array.from({length:n},(_,i)=>'$'+(i+1)).join(',')}
async function persist(state){if(!enabled())return {enabled:false};return withClient(async c=>{await c.query('BEGIN');try{for(const [table,columns,map] of tables){await c.query(`DELETE FROM ${table}`);for(const x of state[keyMap[table]]||[]){const vals=map(x);await c.query(`INSERT INTO ${table}(${columns}) VALUES(${placeholders(vals.length)})`,vals)}}await c.query("INSERT INTO meta(key,value) VALUES('schemaVersion','5') ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value");await c.query('COMMIT');return {enabled:true}}catch(e){await c.query('ROLLBACK').catch(()=>{});throw e}})}
