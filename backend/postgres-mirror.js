...
async function load(){if(!enabled())return null;return withClient(async c=>{const state={schemaVersion:7};let total=0;for(const [table] of tables){const r=await c.query(`SELECT data FROM ${table}`);const rows=r.rows.map(x=>x.data);state[keyMap[table]]=rows;total+=rows.length}return {state,total}})}
