...
async function persist(state){if(!enabled())return {enabled:false};return withClient(async c=>{await c.query('BEGIN');try{for(const [table,columns,map] of tables){await c.query(`DELETE FROM ${table}`);for(const x of state[keyMap[table]]||[]){const vals=map(x);await c.query(`INSERT INTO ${table}(${columns}) VALUES(${placeholders(vals.length)})`,vals)}}await c.query("INSERT INTO meta(key,value) VALUES('schemaVersion','7') ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value");await c.query('COMMIT');return {enabled:true}}catch(e){await c.query('ROLLBACK').catch(()=>{});throw e}})}
