...
CREATE TABLE IF NOT EXISTS sms_receipts(id TEXT PRIMARY KEY,data TEXT NOT NULL);
...
function writeDb(input,options={}){const d=shape(input),existingNotificationIds=new Set(db.prepare('SELECT id FROM notifications').all().map(r=>r.id));db.exec('BEGIN IMMEDIATE');try{
 ...
 replace('sms_receipts',d.smsReceipts,'INSERT INTO sms_receipts(id,data) VALUES(?,?)',x=>[x.id,JSON.stringify(x)]);
...
function stateCount(d){return ['users','requests','messages','ratings','audit','workflows','verificationRequests','providerLocations','deviceSessions','notifications','smsReceipts'].reduce((n,k)=>n+(d[k]?.length||0),0)}
