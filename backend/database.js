...
function writeDb(input,options={}){const d=shape(input),existingNotificationIds=new Set(db.prepare('SELECT id FROM notifications').all().map(r=>r.id));db.exec('BEGIN IMMEDIATE');try{
 ...
 if(!options.suppressMirror&&MIRROR_MODE!=='off'&&mirror.enabled()){if(MIRROR_MODE==='mirror'&&!postgresOperational){console.warn(JSON.stringify({event:'postgres_mirror_write_skipped',reason:'mirror_not_initialized'}));}else mirror.enqueue(d);}
...
async function initDurable(){
 ...
 try{await mirror.initialize();const remote=await mirror.load(),local=readDb(),localCount=stateCount(local),remoteCount=remote?.total||0;if(MIRROR_MODE==='durable'){if(remoteCount>0){writeDb(remote.state,{suppressMirror:true,suppressPush:true});const restored=readDb(),restoredCount=stateCount(restored);if(restoredCount!==remoteCount)throw new Error(`PostgreSQL restore verification failed: expected ${remoteCount} records, restored ${restoredCount}`);console.log(JSON.stringify({event:'postgres_restore_complete',records:remoteCount,verified:true}))}else if(localCount>0){await mirror.persist(local);const seeded=await mirror.load(),seededCount=seeded?.total||0;if(seededCount!==localCount)throw new Error(`PostgreSQL seed verification failed: expected ${localCount} records, persisted ${seededCount}`);console.log(JSON.stringify({event:'postgres_seed_complete',records:localCount,verified:true}))}}else console.log(JSON.stringify({event:'postgres_mirror_ready',localRecords:localCount,remoteRecords:remoteCount,remoteCounts:remote?.counts||{}}));postgresOperational=true;postgresError=null;return {mode:MIRROR_MODE,postgres:true,localRecords:localCount,remoteRecords:remoteCount};}catch(e){postgresOperational=false;postgresError=e.message;console.error(JSON.stringify({event:'postgres_init_failed',mode:MIRROR_MODE,message:e.message}));return {mode:MIRROR_MODE,postgres:false,error:e.message}}
}
...
function dbInfo(){return {engine:MIRROR_MODE==='off'?'sqlite':postgresOperational?'sqlite+postgres-mirror':'sqlite',schemaVersion:SCHEMA_VERSION,file:SQLITE,mirrorMode:MIRROR_MODE,postgresConfigured:mirror.enabled(),postgresOperational,postgresError}}
