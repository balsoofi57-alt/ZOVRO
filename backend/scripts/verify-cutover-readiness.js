...
const TABLES=['sms_receipts','users','service_requests','messages','ratings','audit_log','workflow_records','verification_requests','provider_locations','device_sessions','notifications'];
const PK={sms_receipts:'id',users:'id',service_requests:'id',messages:'id',ratings:'id',audit_log:'id',workflow_records:'id',verification_requests:'id',provider_locations:'provider_id',device_sessions:'id',notifications:'id'};
...
(async()=>{
 ...
 try{
  ...
  if(result.schemaVersion!=='7')result.mismatches.push('schemaVersion');
  ...
  result.ok=result.schemaVersion==='7'&&result.nonEmpty&&result.mismatches.length===0;
