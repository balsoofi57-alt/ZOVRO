...
(async()=>{
  ...
  try {
    ...
    const schema = missing.includes('meta') ? {rows:[]} : await client.query("select value from meta where key='schemaVersion'");
    const counts = {};
    for (const table of REQUIRED_TABLES.filter(t=>t!=='meta'&&names.has(t))) {
